...
$pageTitle = "Add New Product — Velyora Admin";
$successMsg = '';
$errorMsg = '';

// Fetch all active categories for the dropdown menu
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    ...
    // If sale price is empty, save as NULL in the database
    ...
    if (empty($name) || $category_id === 0 || $price <= 0) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($fileExt, $allowedExts)) {
                // Generate a unique filename (e.g., product-1698765432-8473.jpg)
        ...
        // Insert into Database if no errors occurred during upload
        if (empty($errorMsg)) {
            try {
                $stmt = $pdo->prepare("INSERT INTO products (category_id, name, description, price, sale_price, stock_qty, image, is_featured, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$category_id, $name, $description, $price, $sale_price, $stock_qty, $imageName, $is_featured, $status]);
                    ...
                    <div style="display: flex; flex-direction: column; gap: 20px;">
                        ...
                        <div class="card">
                            ...
                            <div style="margin-bottom: 15px;">
                                ...
                                <input type="text" name="name" required placeholder="e.g. Premium Wireless Headphones" style="width: 100%; height: 40px; padding: 0 12px; border: 1px solid var(--color-border); border-radius: var(--radius-sm); outline: none; font-family: inherit;">
                            </div>

                            <div style="margin-bottom: 15px;">
                                <label style="display: block; font-size: 12px; font-weight: 700; color: var(--color-text-main); margin-bottom: 6px;">Description</label>
                                <textarea name="description" rows="5" placeholder="Describe the product..." style="width: 100%; padding: 12px; border: 1px solid var(--color-border); border-radius: var(--radius-sm); outline: none; font-family: inherit; resize: vertical;"></textarea>
                            </div>
                        </div>
                        ...
                        <div class="card">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="price" required placeholder="0.00" style="width: 100%; height: 40px; padding: 0 12px; border: 1px solid var(--color-border); border-radius: var(--radius-sm); outline: none; font-family: inherit;">
                                </div>
                                <div>
                                    <label style="display: block; font-size: 12px; font-weight: 700; color: var(--color-text-main); margin-bottom: 6px;">Sale Price (Rs.)</label>
                                    <input type="number" step="0.01" name="sale_price" placeholder="Leave empty if no discount" style="width: 100%; height: 40px; padding: 0 12px; border: 1px solid var(--color-border); border-radius: var(--radius-sm); outline: none; font-family: inherit;">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="stock_qty" value="0" style="width: 100%; height: 40px; padding: 0 12px; border: 1px solid var(--color-border); border-radius: var(--radius-sm); outline: none; font-family: inherit;">
                            </div>
                        </div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 20px;">
                        ...
                        <div class="card">
                            ...
                            <label style="display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: 600; color: var(--color-text-main); cursor: pointer;">
                                <input type="checkbox" name="is_featured" value="1" style="width: 16px; height: 16px; accent-color: var(--color-primary);">
                                Feature on Homepage
                            </label>
                        </div>
                        ...
                        <div class="card">
                            ...
                            <div>
                                <label style="display: block; font-size: 12px; font-weight: 700; color: var(--color-text-main); margin-bottom: 6px;">Upload Image (JPG, PNG, WEBP)</label>
                        ...
                        <button type="submit" name="add_product" class="btn-create-pill" style="width: 100%; justify-content: center; height: 48px; font-size: 14px;">
